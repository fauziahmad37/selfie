		  ...
		  <div cla  ss="row">
		  	<div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <div class="x_content">
		  			<form id="demo-form2" autocomplete="off" data-parsley-validate class="form-horizontal form-label-left" action="<?php echo site_url('/C_setoran/data_hutang_armada'); ?>" method="post">
					  ...
					  <div class="form-group">                   	
						<label class="control-label col-md-2 col-sm-2 col-xs-2" for="no_pintu">Masukan NO KIP<span class="required">*</span>
						</label>
						<div class="col-md-2 col-sm-2 col-xs-2">
						  <input type="text"  name="no_kip" required="required" />
						</div>                      
					  </div>
					...
					<table id="example" class="display nowrap" style="width:100%">
				        <thead>
				            <tr>
				                ...
				                <th>AR KS SALDO AWAL</th>
				                <th>AR KS BULAN INI PLUS</th>
				                <th>AR KS BULAN INI MINUS</th>
				                <th>AR KS SALDO AKHIR</th>
				                <th>AR TABUNGAN SALDO AWAL</th>
				                <th>AR TABUNGAN BULAN INI PLUS</th>
				                <th>AR TABUNGAN BULAN INI MINUS</th>
				                <th>AR TABUNGAN SALDO AKHIR</th>
				                <th>AR LAIN SALDO AWAL</th>
				                <th>AR LAIN BULAN INI PLUS</th>
				                <th>AR LAIN BULAN INI MINUS</th>
				                <th>AR LAIN SALDO AKHIR</th>
				            </tr>
				        </thead>
				        <tbody>
				            <?php
							foreach ($data AS $row) { 
								echo '<tr>'.
										...
										'<td>'.$row['AR_KS_SALDO_AWAL'].'</td>'.
										'<td>'.$row['AR_KS_BULAN_INI_PLUS'].'</td>'.
										'<td>'.$row['AR_KS_BULAN_INI_MINUS'].'</td>'.
										'<td>'.$row['AR_KS_SALDO_AKHIR'].'</td>'.

										'<td>'.$row['AR_TABSP_SALDO_AWAL'].'</td>'.
										'<td>'.$row['AR_TABSP_BULAN_INI_PLUS'].'</td>'.
										'<td>'.$row['AR_TABSP_BULAN_INI_MINUS'].'</td>'.
										'<td>'.$row['AR_TABSP_SALDO_AKHIR'].'</td>'.

										'<td>'.$row['AR_LAIN_SALDO_AWAL'].'</td>'.
										'<td>'.$row['AR_LAIN_BULAN_INI_PLUS'].'</td>'.
										'<td>'.$row['AR_LAIN_BULAN_INI_MINUS'].'</td>'.
										'<td>'.$row['AR_LAIN_SALDO_AKHIR'].'</td>'.
								'</tr>';
								}
							?>
				        </tbody>
