...
// 				console.info(pool);
				...
				if(pool.pool_area < 3){
					var data = {
						labels: ["Operating", "KS", "Trips", "HK Car"],
						fontSize : 20,
						datasets: [{
							...
							data: [(pool.ops).toFixed(2), (pool.ks).toFixed(2), (pool.rit).toFixed(2), (pool.hk_car).toFixed(2)]
						}]
					};
					div = div + '<tr><td>Operating</td><td>'+numberWithCommas(pool.target_ops)+'</td><td>'+numberWithCommas(pool.act_ops)+'</td><td>'+(pool.ops).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td>KS</td><td>'+numberWithCommas(pool.target_ks)+'</td><td>'+numberWithCommas(pool.act_ks)+'</td><td>'+(pool.ks).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td>Trips</td><td>'+numberWithCommas(pool.target_rit)+'</td><td>'+numberWithCommas(pool.act_rit)+'</td><td>'+(pool.rit).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td>HK Car</td><td>'+numberWithCommas(pool.target_car)+'</td><td>'+numberWithCommas(pool.act_car)+'</td><td>'+(pool.hk_car).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td colspan="2">Overall</td><td colspan="2">'+(pool.overall).toFixed(2)+'%</td></tr>';
				} else {
					var data = {
						labels: ["Operating", "ARPU", "Trips", "HK Driver"/*, "HK Car"*/],
						fontSize : 20,
						datasets: [{
							backgroundColor: "rgba(3, 88, 106, 0.2)",
							borderColor: "rgba(3, 88, 106, 0.80)",
							...
							pointHoverBackgroundColor: "#fff",
							pointHoverBorderColor: "rgba(220,220,220,1)",
							data: [(pool.ops).toFixed(2), (pool.rev).toFixed(2), (pool.rit).toFixed(2), (pool.hk_driver).toFixed(2)/*, (pool.hk_car).toFixed(2)*/]
						}]
					};
					div = div + '<tr><td>Operating</td><td>'+numberWithCommas(pool.target_ops)+'</td><td>'+numberWithCommas(pool.act_ops)+'</td><td>'+(pool.ops).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td>ARPU</td><td>'+numberWithCommas(pool.target_rev)+'</td><td>'+numberWithCommas(pool.act_rev)+'</td><td>'+(pool.rev).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td>Trips</td><td>'+numberWithCommas(pool.target_rit)+'</td><td>'+numberWithCommas(pool.act_rit)+'</td><td>'+(pool.rit).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td>HK Driver</td><td>'+numberWithCommas(pool.target_driver)+'</td><td>'+numberWithCommas(pool.act_driver)+'</td><td>'+(pool.hk_driver).toFixed(2)+'%</td></tr>';
					div = div + '<tr><td colspan="2">Overall</td><td colspan="2">'+(pool.overall).toFixed(2)+'%</td></tr>';
				}
				div = div + '</tbody></table>';									
