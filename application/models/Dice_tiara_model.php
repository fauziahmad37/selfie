...
class Dice_tiara_model extends CI_Model{
	private $db_local;
	private $db_name = 'dice_tiara';
	...
	function data($date = ''){
		...
		$data['operasi'] = $this->db_local->query("select id_pool_mobil, id_jam_operasional, count(*) as ct from spj_operasional where tgl_spj = '".$date."' AND a.status::text != 'CANCELED'::text group by id_pool_mobil, id_jam_operasional order by id_pool_mobil")->result_array();
		$data['status_all'] = $this->db_local->query("select id_pool, master_pool.nama as nama, status, count(*) as ct from mobil_pool join master_pool on master_pool.id = mobil_pool.id_pool group by id_pool, nama, status order by id_pool;")->result_array();
		$this->db_local->close();
		return $data;
	}
	...
	function revenues($start, $end){
		$this->db_local = $this->load->database($this->db_name, TRUE);
		$data = array();
		$query = $this->db_local->query("select x.tgl_spj, x.id_pool_mobil, total_spj, total_gross, 
			(total_komisi_argo + total_komisi_trip + total_komisi_argo_bersih + nominal_insentif_kehadiran) as total_komisi, total_bbm, total_denda, 
			total_lain, hutang_baru, bayar_hutang, nominal_insentif_kehadiran, 
			(total_gross - total_komisi_argo - total_komisi_trip - total_komisi_argo_bersih - total_bbm - hutang_baru + total_lain - bayar_hutang - nominal_insentif_kehadiran) as total_rev  
			from (select tgl_spj, id_pool_mobil, count(id_pool_mobil) as total_spj, 
			sum(argo_setelah_adj) as total_gross, sum(total_komisi) as total_komisi_argo, sum(nominal_insentif_kehadiran) as nominal_insentif_kehadiran,
			sum(komisi_argo_bersih) as total_komisi_argo_bersih, sum(komisi_trip) as total_komisi_trip
			from faktur  
			...
			join (
			select tgl_spj, id_pool_mobil, sum(case when id_komponen_setoran > 3 then -nominal else 0 end) as total_lain from komponen_setoran_faktur 
			...
			join (select tgl_spj, id_pool_mobil, sum(nominal) as credit from metode_pembayaran_faktur join spj_operasional on id_faktur = spj_operasional.id 
			and tgl_spj >= '".$start."' and tgl_spj <= '".$end."' 
			group by id_pool_mobil, tgl_spj)
			f on f.tgl_spj = x.tgl_spj and f.id_pool_mobil = x.id_pool_mobil
	...
	function get_detail_unit($id_pool, $date){
		...
		$data = $this->db_local->query("select nomor_spj, nama, no_kip, nomor_pintu, (id_jam_operasional - 1) as jenis_ops, argo_setelah_adj, total_komisi, total_bbm, liter, 
			a.bayar_hutang, a.hutang_baru, denda_faktur.adjusment as bayar_denda, lain, d.speedo, d.rit, d.drop, adjustment, nominal_insentif_kehadiran, komisi_trip,
			komisi_argo_bersih, (total_setoran + credit - total_bbm - bayar_hutang) as total_setoran
			...
			join (select id_faktur, sum(case when id_komponen_setoran <= 3 then nominal else 0 end) as adjustment, sum(case when id_komponen_setoran > 3 then nominal else 0 end) lain from komponen_setoran_faktur 
			...
			join (select id_faktur, sum(nominal) as credit from metode_pembayaran_faktur join spj_operasional on id_faktur = spj_operasional.id and tgl_spj = '".$date."'
			and spj_operasional.id_pool_mobil = ".$id_pool." group by id_faktur)
			e on e.id_faktur = id_spj
