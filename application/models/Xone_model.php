...
class Xone_model extends CI_Model{
	private $db_local;
	private $db_name = 'x_one';
	function __construct() {
		parent::__construct();
		
	}
	
	function order($start_dt, $date){
		if($date === '')
		...
		$data['order'] = $this->db_local->query("select thomas, status, count(*) as ct
			from tiara_x1.order where date(created) >= '".$start_dt."' and date(created) <= '".$date."' group by thomas, status order by thomas, status;")->result_array();
		$data['order_cancel'] = $this->db_local->query("select cancel_type, count(status) as ct from tiara_x1.order 
			where date(created) >= '".$start_dt."' and date(created) <= '".$date."' and cancel_type is not null group by cancel_type;")->result_array();
		$data['broadcast'] = $this->db_local->query("select pool_id, name, count(*) as ct, sum(case when a.status = 2 then 1 else 0 end) as accept, 
			...
			where date(a.created) >= '".$start_dt."' and date(a.created) <= '".$date."' group by pool_id, pool.name;")->result_array();
		...
		$data['time'] = $this->db_local->query("select avg(arrived_time - searching_time) as arrived, 
			avg(board_time - searching_time) as board from tiara_x1.order where date(created) >= '".$start_dt."' and date(created) <= '".$date."';")->result_array();
		$this->db_local->close();	
		return $data;
	}
	...
	function get_trip($date, $reg_no, $tipe){
		...
		$data = $this->db_local->query("select (case when \"order\".thomas = 'T' then 'MyTrip' when \"order\".thomas = 'F' then 'Call Center' else 'Melambai' end) as tipe, 
			...
			and trip.start <= '".$end."' and taxi.reg_no = '".$reg_no."' and fare > 0 order by start;")
			->result_array();
		$this->db_local->close();		
		return $data;
	}
	...
	function get_trip_sum($date, $reg_no, $tipe){
		...
		$data = $this->db_local->query("select count(reg_no) as trip, sum(fare) as argo
			...
			where trip.start >= '".$start."' 
			and trip.start <= '".$end."' and taxi.reg_no = '".$reg_no."' and fare > 0")
			->result_array();	
		return $data;
	}
	...
	function get_status_taxi(){
		...
		$data['status'] = $this->db_local->query("select 
                ...
                join simcard on simcard.id = Taxi.simcard_id;")->result_array();
	...
	function shelter_ritase($start, $end, $lat, $lng, $radius){
		$data = $this->db_local->query("Select date(start) as tgl, coalesce(count(*),0) as ritase, coalesce(sum(fare), 0) as argo from trip 
			where fare > 0 and date(start) >= '".$start."' and date(start) <= '".$end."'
			and earth_box(ll_to_earth(".$lat.",".$lng."), ".$radius.") @> (ll_to_earth(start_lat, start_lng))
	...
	function get_rds_status(){
		...
		select taxi.pool_id, sum(case when device_status = 1 then 1 else 0 end) as connected, 
					...
					join pool on pool.id = taxi.pool_id where is_active = 1 and ((taxi.pool_id > 1 and taxi.pool_id < 31) or (taxi.pool_id > 31 and taxi.pool_id < 40)) group by taxi.pool_id ) a
					full join 
...
select pool_id, reg_no from log_login 
		...
		join taxi on taxi.dds_id = dds.id where date(log_login.created) = current_date and success = 0 group by pool_id, reg_no) x
	...
	//SPJ
	function get_trip_from_spj($query){
		$this->db_local = $this->load->database($this->db_name, TRUE);
		$data = $this->db_local->query("select assignment_code, count(st) as total_trip, sum(argo) as total_argo from (			
			select assignment_code, max(start) as st, max(fare) as argo 
			from trip join driver_access on driver_access_id = driver_access.id and assignment_code in ($query)
			join (select username, max(attempt_time) as dt from log_login where success = 1 and username in ($query) group by username) x on x.username = assignment_code
			and (start) >= (dt) group by assignment_code, start) x group by assignment_code;")->result_array();
		$this->db_local->close();						
		return $data;
	}
